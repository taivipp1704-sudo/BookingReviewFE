...
export default function CustomerLoginPage({ onLogin, onBack, loginMessage, initialMode = "login" }) {
  ...
  return (
    <AuthShell audience="customer">
      <section className="flex min-h-[620px] w-full flex-col rounded-lg border border-line bg-white p-6 shadow-[0_20px_60px_rgba(16,16,16,.09)] sm:p-9">
        ...
        <h1 className="mt-2 flex min-h-[80px] items-end text-3xl font-black sm:text-4xl">{mode === "register" ? "Tạo tài khoản mới" : "Đăng nhập để tiếp tục"}</h1>
        ...
        <form onSubmit={submit} className="mt-5 grid gap-4 sm:grid-cols-2">
          <label className={`block min-w-0 text-xs font-black text-muted ${mode === "login" ? "invisible pointer-events-none" : ""}`} aria-hidden={mode === "login"}>Họ và tên
          ...
          <label className="block min-w-0 text-xs font-black text-muted">Số điện thoại
