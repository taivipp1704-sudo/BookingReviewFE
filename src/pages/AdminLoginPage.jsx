...
export default function AdminLoginPage({ onLogin, onBack }) {
  ...
  return (
    <AuthShell audience="admin">
      <form onSubmit={submit} className="w-full rounded-lg border border-line bg-white p-6 shadow-[0_20px_60px_rgba(16,16,16,.09)] sm:px-9 sm:pb-[50px] sm:pt-9">
        ...
        <div className="mt-10 grid h-14 w-14 place-items-center rounded-lg bg-ink text-acid shadow-lg"><LockKeyhole className="h-5 w-5" /></div>
        ...
        <div className="mt-10 space-y-5">
          <label className="block text-xs font-black text-muted">Email
            <span className="mt-2 flex h-[58px] items-center gap-3 rounded-lg border border-line bg-paper px-4 focus-within:border-ink">
        ...
        <button disabled={busy} className="mt-7 flex h-[60px] w-full items-center justify-center gap-3 rounded-lg bg-ink px-4 text-xs font-black uppercase tracking-wider text-acid disabled:opacity-50">
